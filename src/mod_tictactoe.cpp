...
public:
    ...
    static std::string GetIconePathBySize(std::string iconName, uint32 size)
    {
    ...
    static std::string GetIconeNameByPlayerTheme(std::vector<std::string> Path_Icon, uint32 playerId, uint32 themeId)
    {
        std::string iconName = "";
                    ...
                    case WAIT_HUMAN_PLAYER_02:
                    {
                        if (!NpcTicTacToeData.empty())
                        {
                            ...
                            for (std::map<ObjectGuid, NpcTicTacToeInfo>::const_iterator i = NpcTicTacToeData.begin(); i != NpcTicTacToeData.end(); ++i)
                            {
                                ...
                                {
                                    Player* playerFind02 = ObjectAccessor::FindConnectedPlayer(i->second.guidPlayer02);
                                    if (playerFind02)
                                    {
    ...
    static char AIcheck_win(NpcTicTacToeInfo const* NpcTicTacToeGameExist)
    {
        ...
        if (NpcTicTacToeGameExist->AIgameGrid[0][0] != 0 &&
            (
                (NpcTicTacToeGameExist->AIgameGrid[0][0] == NpcTicTacToeGameExist->AIgameGrid[0][1] && NpcTicTacToeGameExist->AIgameGrid[0][0] == NpcTicTacToeGameExist->AIgameGrid[0][2]) ||
        ...
        if (NpcTicTacToeGameExist->AIgameGrid[1][1] != 0 &&
            (
                (NpcTicTacToeGameExist->AIgameGrid[1][1] == NpcTicTacToeGameExist->AIgameGrid[1][0] && NpcTicTacToeGameExist->AIgameGrid[1][1] == NpcTicTacToeGameExist->AIgameGrid[1][2]) ||
        ...
        if (NpcTicTacToeGameExist->AIgameGrid[2][2] != 0 &&
            (
    ...
    static bool OnGossipSelectTicTacToeCumputer(Player* player, Creature* creature, uint32 /* sender */, uint32 /* action */, NpcTicTacToeInfo const* NpcTicTacToeGameExistFound)
    {
        ...
        if ((int)aiFoundI != -1 && (int)aiFoundJ != -1) // TODO: should be always true
        {
            ...
            creature->Whisper(whisperMessage.c_str(), LANG_UNIVERSAL, player, true);
        ...
        if (checkIfGameWinOrDraw(NpcTicTacToeGameExist) == false)
        {
            ...
            if (NpcTicTacToeGameExist->gameTypeSelected != 1 && NpcTicTacToeGameExist->nextIdPlayerMove == 2)
            {
                ...
                if (action == (GOSSIP_ACTION_INFO_DEF + 76))
                {
                    ...
                    ENSURE_AI(npc_tic_tac_toe::npc_tic_tac_toeAI, creature->AI())->_events.ScheduleEvent(WAIT_COMPUTER_PLAYER_TEXT_01, 1s);
                    ENSURE_AI(npc_tic_tac_toe::npc_tic_tac_toeAI, creature->AI())->_events.ScheduleEvent(WAIT_COMPUTER_PLAYER_TEXT_02, 3s);
                    ENSURE_AI(npc_tic_tac_toe::npc_tic_tac_toeAI, creature->AI())->_events.ScheduleEvent(WAIT_COMPUTER_PLAYER_TEXT_03, 5s);
                }
                ...
                if (NpcTicTacToeGameExist->gameStatus == 20)
                {
                    ...
                    if (NpcTicTacToeGameExist->gameTypeSelected == 1)
                    {
                        ...
                        if (NpcTicTacToeGameExist->nextIdPlayerMove == 2)
                        {
                            ...
                            playerFind01 = ObjectAccessor::FindConnectedPlayer(NpcTicTacToeGameExist->guidPlayer01);
                            if (playerFind01)
                            {
                                ChatHandler(playerFind01->GetSession()).PSendSysMessage("%s", notificationMessage.c_str());
                ...
                if (NpcTicTacToeGameExist->gameStatus == 30)
                {
                    ...
                    if (NpcTicTacToeGameExist->gameTypeSelected == 1)
                    {
                        ...
                        if (NpcTicTacToeGameExist->nextIdPlayerMove == 1 || NpcTicTacToeGameExist->nextIdPlayerMove == 2)
                        {
                            ...
                            if (playerFind01)
                            {
                                ChatHandler(playerFind01->GetSession()).PSendSysMessage("%s", "This game it's a draw.");
                            ...
                            if (playerFind02)
                            {
                                ChatHandler(playerFind02->GetSession()).PSendSysMessage("%s", "This game it's a draw.");
                    ...
                    else if (NpcTicTacToeGameExist->gameTypeSelected == 2)
                    {
                        ...
                        if (NpcTicTacToeGameExist->nextIdPlayerMove == 1 || NpcTicTacToeGameExist->nextIdPlayerMove == 2)
                        {
                            ChatHandler(player->GetSession()).PSendSysMessage("%s", "This game it's a draw.");
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        SendGossipMenuFor(player, GOSSIP_MENU_TICTACTOE_WELCOME, creature->GetGUID());
        return true;
    }
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /* sender */, uint32 action) override
    {
        ...
        switch (action)
        {
            ...
            case GOSSIP_ACTION_INFO_DEF + 10:
            {
                ...
                AddGossipItemFor(player, GOSSIP_ICON_TALK, "\nMain menu", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
                SendGossipMenuFor(player, GOSSIP_MENU_TICTACTOE_EMPTY, creature->GetGUID());
            } break;
            ...
            case GOSSIP_ACTION_INFO_DEF + 15:
            {
                ...
                AddGossipItemFor(player, GOSSIP_ICON_TALK, "\nMain menu", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
                SendGossipMenuFor(player, GOSSIP_MENU_TICTACTOE_SETUP, creature->GetGUID());
            } break;
            ...
            case GOSSIP_ACTION_INFO_DEF + 54:
            {
                ...
                // std::string namePlayer01 = player->GetName().c_str();
